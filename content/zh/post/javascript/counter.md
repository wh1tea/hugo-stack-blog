...
- **`button type="button"`**：按钮默认 `type="submit"`，放进 `<form>` 会触发提交刷新页面，下面是演示代码。显式声明是防御习惯。
  ...
  ```html
      ...
      <p>
        Click the first button: The page will refresh, and the address bar will
        show
        <code>?q=xxx</code> parameter (submission successful)<br />
        Click the second button: Nothing will happen
      </p>  
  ```
详见[Your_first_form](https://developer.mozilla.org/zh-CN/docs/Learn_web_development/Extensions/Forms/Your_first_form)

- [`aria-label`](https://developer.mozilla.org/zh-CN/docs/Web/Accessibility/ARIA/Reference/Attributes/aria-label) 为无可见文本的交互元素（如图标按钮）提供屏幕阅读器专用的语音名称，并会覆盖元素自身的文本、`alt` 或关联 `<label>`，优先作为无障碍标签。
...
同时在CSS里面添加 `.positive` / `.negative` / `.zero` 三种颜色类，使用 JS 切换类名。
...
4. **`classList.toggle(类名, 布尔)`**：第二个参数是"强制开关"，`currentCount > 0` 为真就加类、为假就移除。三个类互斥，正好表达正/负/零三态。`.zero` 显式设黑看似多余（默认就是黑），但它保证三态配色恒定：以后 `body` 的继承文字色无论怎么变，正绿、负红、零黑都不会漂移。
...
- **启动即恢复**：`loadCount(true)` 放在脚本末尾，页面一打开就回到上次 Save 的数值，`Load` 按钮则用于运行中手动恢复。
...
1. `<output>` 是 HTML5 表单元素，语义为"计算或用户操作的结果"，正好对应计数器。它的隐式 ARIA 角色是 `role="status"`，而 status 角色自带 `aria-live="polite"` 的实时播报行为——值变化时读屏软件会自动读出，`<div>` / `<span>` 没有这些语义，改了也不播报。它还支持 `for` 属性关联触发它的表单控件。代码里在 `<output>` 上又显式写了 `aria-live="polite"`，属于冗余但无害的双保险，教学上更直白（对比[猜数字游戏](guess-game-tutorial.md)那篇用 `<output>` 直接取代显式 `aria-live` 的更简写法）。
...
3. 容器 `max-width: 300px`，手机竖屏下按钮容易换行。如果希望按钮在窄屏**水平滚动**而不是换行，怎么改？更通用的响应式思路呢？
4. 五个按钮 hover 样式完全一样（变 `lightgray`），UX 上有什么缺点？如何按操作类型区分？

**参考答案与解析**

3. 先给按钮容器加个类名（原代码的 `<div>` 没有类，是隐患），再：
...
4. 五个按钮分属三种职责：**数值操作**（`+` `−`）、**破坏性操作**（`Reset` 清空计数）、**持久化操作**（`Save` / `Load`）。hover 全部灰成一片，用户无法预判操作风险——误触 `Reset` 会丢掉当前计数。
...
5. `setCount` 里的 `if (currentCount !== newValue)` 有什么好处？代码中是否存在绕过 `setCount` 直接改 `currentCount` 的路径？
6. `loadCount` 在本地无数据（`saved === null`）时保持当前值不变、只提示 "No saved data found"。这个行为合理吗？如果希望"加载"时无数据则归零，怎么改？

**参考答案与解析**

5. 好处是**避免无意义的 DOM 写入**：值没变时 `textContent` 赋值和三次 `classList.toggle` 都是白做。而且它让"加载无数据"这种场景天然不触发重渲染。逐条检查写入点：`incrementCount` / `decrementCount` / `resetCount` / `loadCount` 全部是先算新值再调 `setCount`，**没有任何绕过**。`currentCount` 是唯一状态源，`setCount` 是唯一入口，"状态必与界面同步"的不变量由此成立。

6. 合理——"加载"的语义是"恢复已保存的值"，从未保存过就不该动界面，这正是产品语义的边界。如果你想要的是"**加载默认值**"语义（无数据时归零），不需要碰 `localStorage`，只需改变量初值：
...
7. 为什么用 `Number.isNaN` 而不是全局 `isNaN`？`Number.isFinite` 缺失会埋下什么雷？换成 `JSON.parse` 解析存储值会怎样？
8. `loadCount` 的 `catch` 分支里有一处重复赋值，你发现了吗？有什么问题？

**参考答案与解析**

7. 两者最核心的区别是**是否先做类型转换**：
...
8. 发现了——`catch` 里先给 `msg` 赋了带具体错误信息的字符串（`Load failed: ...`，含 `e.message`），紧接着又被下一行 `msg = "Failed to load count. Please check storage permissions.";` **覆盖**，第一行成了永不生效的死代码。问题在于：开发期最需要的具体错误信息（`e.message`）被丢掉了，用户只看到笼统提示。修复是删掉其中一行，或合并成更细粒度的消息（如 `Load failed: ${e.message}`）。这正是"写了 try-catch 但没走查一遍"的典型漏网之鱼。

### 第五轮：事件绑定与代码组织

9. 为什么 `Load` 按钮绑 `() => loadCount(false)`，而 `Save` 直接绑 `saveCount`？可以直接绑 `loadCount` 吗？
10. 所有函数都挂在全局作用域。若要嵌入大项目或写单元测试，如何重构得更模块化、可测试？给出两种方案。

**参考答案与解析**

9. 直接绑会出 bug，原因是 `addEventListener` 的回调**总会收到一个 `Event` 对象作为第一个实参**：
...
`saveCount` 不接收参数，直接绑定时多收一个事件对象也无害，所以两种写法都行。规律是：**回调期望的签名与函数参数对不上时，就用箭头函数包一层或 `bind` 预置参数**。

> 常见误区：把带参函数名直接传给 `addEventListener`，参数被事件对象悄悄覆盖——这是"看起来能跑、行为全错"的隐蔽 bug。

10. 全局作用域的问题：所有变量/函数挂到 `window` 上互相污染；函数隐式共享状态，无法单独测试（测 `renderCount` 得先造出 DOM）；脚本加载顺序敏感；换个页面无法复用。两种重构方案：
...
- [MDN `<output>` 元素](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/output) 计算结果语义与 `for` 属性
- [MDN `aria-live`](https://developer.mozilla.org/zh-CN/docs/Web/Accessibility/ARIA/Attributes/aria-live) polite 与 assertive 的播报差异
- [MDN `localStorage`](https://developer.mozilla.org/zh-CN/docs/Web/API/Window/localStorage) 字符串存取与 SecurityError
- [MDN `Number.isNaN` / `isFinite`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Number/isNaN) 无类型转换的数值判断
- [MDN `addEventListener`](https://developer.mozilla.org/zh-CN/docs/Web/API/EventTarget/addEventListener) 回调参数传递规则
- [MDN `Function.prototype.bind`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/Function/bind) 预置函数参数
