...
- PowerShell 会把 git 输出到 stderr 的进度信息（如 `Everything up-to-date`）渲染成红字，属正常现象，判断成败看退出码

## 结语

GitHub 推送失败不要急着怀疑账号，先按「身份 → 凭据 → 网络 → 配置」分层排查：身份缺失补 `user.name` / `user.email`；凭据缺失在 WSL 下优先复用 Windows 的 Git Credential Manager（包装脚本解决路径空格）；TLS 握手失败用 `curl` 对比直连与代理，确认是代理问题后按域名跳过即可；共享仓库两侧共用 local 配置，WSL 专属设置要挪到 WSL 全局，避免 Windows 侧误报。本次几个问题都在这套思路下几分钟内定位，推送链路从此稳定。

## 参考

- [Git 远程推送失败排查（认证与合并）](git-push-existing-remote.md)
- [GitHub SSH 认证配置](git-github-ssh.md)
- [WSL2 安装与使用完全指南](../linux/wsl2.md)
